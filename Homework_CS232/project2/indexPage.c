...
struct trieNode* indexPage(const char* url); //Need to change this to return a root node

int addWordOccurrence(const char* word, const int wordLength, struct trieNode* root); //added root into params
...
int main(int argc, char** argv){
  ...
  //error handling
  if (argc > 2){ 
    fprintf(stderr, "Please correct input: %s [url]", argv[0]);
  } 
